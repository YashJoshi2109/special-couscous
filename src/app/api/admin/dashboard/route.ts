...
export async function GET(request: NextRequest) {
  ...
  const estimatedPayrollPeriod = periodSessions.reduce((sum, s) => {
    if (!s.clockOutTime || !s.user.employee) return sum
    const hours = (s.clockOutTime.getTime() - s.clockInTime.getTime()) / (1000 * 60 * 60)
    return sum + hours * s.user.employee.hourlyRate + (s.tipsAmount ?? 0) + (s.bonusAmount ?? 0)
  }, 0)

  const totalHoursToday = completedToday.reduce((sum, s) => {
    if (!s.clockOutTime) return sum
    return sum + (s.clockOutTime.getTime() - s.clockInTime.getTime()) / (1000 * 60 * 60)
  }, 0)

  const liveEmployees = openSessions.map((session) => ({
    id: session.user.id,

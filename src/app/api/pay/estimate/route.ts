...
export async function GET(request: NextRequest) {
  ...
  const basePay = sessions.reduce((sum, s) => {
  ...
  const tips = sessions.reduce((sum, s) => sum + (s.tipsAmount ?? 0), 0)
  const roomBonus = sessions.reduce((sum, s) => sum + (s.bonusAmount ?? 0), 0)
  ...
  const voucherBonus = vouchers.reduce((sum, v) => sum + v.bonusAmount, 0)
